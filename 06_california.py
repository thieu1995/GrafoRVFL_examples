...
from data_util import get_california_housing
...
def run_trial(model, seed, data, param_bounds, cf):
    ...
    # Initialize model
    tuner = GfoRvflTuner(problem_type=cf.PROBLEM_REG, bounds=param_bounds, cv=cf.CV, scoring=cf.LOSS_REG,
    ...
    res = tuner.best_estimator.evaluate(y_test, y_pred, list_metrics=cf.LIST_METRIC_REG)
...
if __name__ == "__main__":
    ...
    Path(f"{Config.PATH_SAVE}/{Config.DATA_05}").mkdir(parents=True, exist_ok=True)
    ...
    df_loss.to_csv(f"{Config.PATH_SAVE}/{Config.DATA_05}/{Config.RESULT_LOSS}", index=False, header=True)
    df_result.to_csv(f"{Config.PATH_SAVE}/{Config.DATA_05}/{Config.RESULT_METRICS}", index=False, header=True)
    df_best_params.to_csv(f"{Config.PATH_SAVE}/{Config.DATA_05}/{Config.RESULTS_PARAMS}", index=False, header=True)

    print(f"Done with data: {Config.DATA_05}.")
