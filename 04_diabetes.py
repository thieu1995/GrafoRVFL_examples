...
from data_util import get_diabetes
...
if __name__ == "__main__":
    ...
    Path(f"{Config.PATH_SAVE}/{Config.DATA_04}").mkdir(parents=True, exist_ok=True)
    ...
    df_loss.to_csv(f"{Config.PATH_SAVE}/{Config.DATA_04}/{Config.RESULT_LOSS}", index=False, header=True)
    df_result.to_csv(f"{Config.PATH_SAVE}/{Config.DATA_04}/{Config.RESULT_METRICS}", index=False, header=True)
    df_best_params.to_csv(f"{Config.PATH_SAVE}/{Config.DATA_04}/{Config.RESULTS_PARAMS}", index=False, header=True)

    print(f"Done with data: {Config.DATA_04}.")
